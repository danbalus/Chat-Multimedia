...
public class Download extends java.util.Observable implements FileChatConstants
{
    ...
    byte[] tmp = new byte[FileChat.monitor.getDownloadSpeedLimit()]; 
    ...
    public boolean downloadFile()
    {
        ...
        for (File file : files) {
            ...
            try 
            {
                ...
                if(!FileChat.monitor.isOverRideFiles()&& myFile.canRead()) 
                	// checks if override file is open or not. If its opened deletes existing one and downloads it again. If its flase aborts current files download goes next.
                {
